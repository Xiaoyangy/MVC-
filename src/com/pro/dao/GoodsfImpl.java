...
public class GoodsfImpl implements GoodsfDao {

	@Override
	public int addGood(Goodsf good) throws Exception {
		Connection conn = DBUtils.getConnection();
		String sql = "insert into goodsf(goodsname,image,price,count,date,comment,userid) value(?,?,?,?,?,?,?)";
	...
	public List<Goodsf> findGoodsByUseidAndGoodsname(Goodsf goods)
		...
		List<Goodsf> list = new ArrayList<Goodsf>();
		while (rs.next()) {
			Goodsf goods2 = new Goodsf();
	...
	@Override
	public List<Goodsf> findAllGoods(int userid, int page, int number)
			throws Exception {
		Connection conn = DBUtils.getConnection();
	...
	public static void main(String[] args) {
		GoodsfImpl goodsfImpl=new GoodsfImpl();
		Users users=new Users();
		users.setId(4);
		Goodsf good=new Goodsf("123456", "www.jpg", 88.8, 20, new Date().toLocaleString(), 0, users, "test");
		good.setId(7);
		try {
			System.out.println(goodsfImpl.modifyGood(good));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	//撤销商品
		...
		@Override
		//前台显示所有商品
		public List<Goodsf> findAllGoods(int page, int number)
				throws Exception {
			Connection conn = DBUtils.getConnection();
